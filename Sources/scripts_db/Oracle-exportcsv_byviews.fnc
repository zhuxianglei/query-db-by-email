...
cursor cur_viewsexception is select owner, object_name as view_name from dba_objects
          where owner = 'QUERY_RO'and object_type like 'VIEW' and object_name like 'UID%FILE%V' and last_ddl_time < sysdate - 1;
begin
  if upper(powner)='SYS' or upper(powner)='SYSTEM' then
    dbms_output.put_line('forbid owner:'||powner);
  else
    for c1 in cur_viewsexception loop
  ...
  if v_vcnt=0 then
    return 'sql file exception,cannot execute in sqlplus,viewcnt:'||to_char(v_vcnt);
  else
    return 'OK';
  end if;
end;
